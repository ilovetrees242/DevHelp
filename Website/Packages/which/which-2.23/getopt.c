...
static void __attribute__((unused)) store_args_and_env(int argc, char *const *argv)
{
...
static void exchange(argv) char **argv;
{
  ...
  if (nonoption_flags_len > 0 && top >= nonoption_flags_max_len)
  {
    ...
    else
    {
      ...
      memset(&new_str[nonoption_flags_max_len], '\0', top + 1 - nonoption_flags_max_len);
    ...
    if (nonoption_flags_max_len == 0)
    {
      ...
      else
      {
        ...
        else
        {
          ...
          memset(&__getopt_nonoption_flags[len], '\0', nonoption_flags_max_len - len);
        }
      }
    }
